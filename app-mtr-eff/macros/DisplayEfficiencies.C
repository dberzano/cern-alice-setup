...
//
////////////////////////////////////////////////////////////////////////////////
void DisplayEfficiencies(TString cdb = "fulleff", Int_t run = 0) {

  AliCDBManager *man = AliCDBManager::Instance();  
  man->SetDefaultStorage("local://$ALICE_ROOT/OCDB");
  man->SetSpecificStorage("MUON/Calib/TriggerEfficiency",
    Form("local://%s/../cdb/%s", gSystem->pwd(), cdb.Data()));
  ...
  AliMUONTriggerEfficiencyCells *effCells =
    (AliMUONTriggerEfficiencyCells *)entry->GetObject();

  AliMUONTriggerChamberEfficiency *eff =
    new AliMUONTriggerChamberEfficiency(effCells);

  // Set my style
  SetStyle();

  // Canvases
  TCanvas *cBend = new TCanvas("cBend", "Efficiencies on bending plane", 500,
    500);
  cBend->Divide(2, 2);
  ...
  for (Int_t i=0; i<4; i++) {
    ...
    hBend[i] = new TH1F(Form("hEffBend_%d", i), Form("Trigger chamber %d", i+1),
      234, 0.5, 234.5);
